...
@onready var arrow_collision_sound: AudioStreamPlayer2D = $Virtual/ArrowCollisionSound;
...
func finish(mob : Mob = null) -> void:
	...
	if self.arrow_collision_sound.playing:
		await self.arrow_collision_sound.finished;
	self.queue_free();
